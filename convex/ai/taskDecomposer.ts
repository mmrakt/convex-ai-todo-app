...
import { type AIRequest, createAIProvider } from '../lib/ai/providers';
import { handleError } from '../lib/base';
import { getApiConfig, OLLAMA_CONFIG, rateLimiter } from './config';
  ...
  handler: async (_ctx, args) => {
    ...
    try {
      ...
      const request: AIRequest = {
        prompt,
        systemPrompt:
          'You are a task decomposition expert. Please break down the given task into small, executable steps.',
        temperature: 0.7,
        maxTokens: 2000,
      };
...
const parseDecompositionResult = (
  content: string,
): Array<{
  title: string;
  description: string;
  ...
  try {
    ...
    return parsed.subtasks.map((task: SubtaskData, index: number) => ({
